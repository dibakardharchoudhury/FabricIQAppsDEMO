...
def fabric_encode_guid(guid: str) -> str:
    """Fabric's Operations-Agent Knowledge id form for an ontology: split the 32 hex digits into
    [8,4,4,4,4,8] chunks and reverse the chunk order. The working portal-generated agent binds the
    ontology data source by this encoded id (e.g. 30f512c4-… -> 4cdeb9b3-…), NOT the plain item id.
    """
    h = guid.replace("-", "")
    c = [h[0:8], h[8:12], h[12:16], h[16:20], h[20:24], h[24:32]]
    r = "".join(reversed(c))
...
def build_configurations(should_run: Optional[bool] = None,
    ...
    re-keyed to this workspace's ontology (the ENCODED Knowledge id + zeros workspaceId, exactly the
    shape the portal writes), the action's pipeline jobArtifactId/jobWorkspaceId point at the
    created Pipe_SendEmailAlert, and the Teams destination is overridden. The full playbook
    (OntologyDefinitions + BAD/UNCERTAIN RuleDefinitions) is pushed verbatim. `identity` is never
    sent — the running user's delegated token provisions Run-as.
    """
    ...
    if datasource_id:
        # Re-key the single Ontology data source to this workspace's ontology using the ENCODED
        # Knowledge id as BOTH key and id, with workspaceId all-zeros — the exact shape the working
        # portal-generated agent uses (and what 'remove + re-add Knowledge' writes back to git).
...
try:
    ...
    # is forced), then ENCODE it to the Knowledge id form the working portal agent uses. Its inner
    # workspaceId stays all-zeros (the portal's own shape); the pipeline action carries the real ws.
    ...
    #    FULL mirror of the working RTI_Demo_OpsAgent_V3 agent: Ontology(encoded Knowledge id + zeros
    #    ws) + Send Email Alert FabricJobAction + Teams + the full BAD/UNCERTAIN playbook. Simpler
    #    configs follow only as a safety net; the first 200 wins.
    def _variant(ds_id=None, with_action=True, with_teams=True, with_playbook=True) -> dict:
        """Build a Configurations.json variant, dropping any excluded component. ds_id is the ENCODED
        Knowledge id; None drops the embedded ontology. with_playbook defaults True (mirror the
