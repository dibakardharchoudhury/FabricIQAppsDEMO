...
| **Connect telemetry → "No Eventhouse found in the workspace"** (STID/GraphQL works) | The Eventhouse is created by **`01_Pipe_Setup`** (RTI_001/002), *not* by Seed & provision. If the app was open before that pipeline ran, its session config cache has no Eventhouse. Just click **Refresh** (top bar) to re‑discover, then Connect telemetry. (Fixed in‑app: `connectTelemetry` now clears the cache per click, like `connectStid`.) If it still can't read data, ensure the signed‑in user has **KQL Database Viewer** on the Eventhouse and the app origin is in the Eventhouse **CORS** allow‑list. |
